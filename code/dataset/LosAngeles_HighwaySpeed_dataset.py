...
def _load_train_raw_data(self):
	...
	raw_data = torch.from_numpy(self.scaler.transform(raw_data).reshape(self.num_timestamp, self.graph_size, self.feature_channels)).float()
	return raw_data
...
def _generate_prediction_raw_mask(self):
	raw_mask = torch.stack(tuple([torch.cat((self.generate_mask_sample(self), self.generate_mask_sample(self)), dim=0) for _ in range(len(self))]), dim=0)
	return raw_mask


def _build_imputation_data(self, raw_data):
	raw_data = [Data(x=raw_data[t], edge_index=self.edge_index, edge_weight=self.edge_weight) for t in range(self.num_timestamp)]
	self.data = []
...
def _build_imputation_mask(self, raw_mask):
	...
	for i in range(len(self)):
		raw_mask_sample = [Data(x=raw_mask[i][t], edge_index=self.edge_index, edge_weight=self.edge_weight) for t in range(self.seq_len)]
		temp_batch = Batch()
...
def _build_prediction_mask(self, raw_mask):
	...
	for i in range(len(self)):
		...
		raw_mask_sample = [Data(x=raw_mask[i][t], edge_index=self.edge_index, edge_weight=self.edge_weight) for t in range(self.seq_len, self.seq_len * 2)]
		temp_batch = temp_batch.from_data_list(raw_mask_sample)
		mask_sample = Data(x=temp_batch.x, edge_index=temp_batch.edge_index, edge_weight=temp_batch.edge_weight)
...
class LosAngelesHighwaySpeed(Dataset):
	def __init__(self, raw_data_dir, raw_adjacency_dir, missing_type, missing_rate, num_timestamp, train_proportion, seq_len, graph_size, feature_channels):
...
class LosAngelesHighwaySpeedImputationTrainSet(LosAngelesHighwaySpeed):
	def __init__(self, raw_data_dir, raw_adjacency_dir, missing_type, missing_rate, num_timestamp, train_proportion, seq_len, graph_size, feature_channels):
		...
		super().__init__(raw_data_dir, raw_adjacency_dir, missing_type, missing_rate, num_timestamp, train_proportion, seq_len, graph_size, feature_channels)
