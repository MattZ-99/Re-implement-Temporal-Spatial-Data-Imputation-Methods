...
class Model(nn.Module):
    ...
    def forward(self, data, direct):
        ...
        mask_zero = torch.zeros(masks[:, 0, :].shape)
        ...
        for t in range(2 * self.seq_len):
            ...
            if (direct_flag and t >= self.seq_len) or (not direct_flag and t < self.seq_len):
                m_predict = mask_zero
            else:
                m_predict = m
            ...
            x_c = m_predict * x + (1 - m_predict) * x_h

            z_h = self.feat_reg(x_c)
            x_loss += torch.sum(torch.abs(x - z_h) * m) / (torch.sum(m) + 1e-5)

            alpha = self.weight_combine(torch.cat([gamma_x, m_predict], dim=1))

            c_h = alpha * z_h + (1 - alpha) * x_h
            x_loss += torch.sum(torch.abs(x - c_h) * m) / (torch.sum(m) + 1e-5)

            c_c = m_predict * x + (1 - m_predict) * c_h

            inputs = torch.cat([c_c, m_predict], dim=1)
