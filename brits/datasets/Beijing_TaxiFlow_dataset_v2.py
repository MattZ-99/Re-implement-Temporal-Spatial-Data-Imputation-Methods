...
def load_train_data(self):
    ...
    self.data = raw_data[:self.num_data_timestamp, :, :, :1]
    L, H, W, F = self.data.shape
    self.data = self.data.reshape(L, H*W*F)
...
class BeijingTaxiFlow(Dataset):
    def __init__(self, raw_data_dir, missing_type, missing_rate, num_timestamp, train_proportion, seq_len, grid_size_x,
        ...
        self.data = torch.stack(tuple([self.data[index:index + self.seq_len, :, :, :] for index in
                                       range(max(self.num_data_timestamp - self.seq_len + 1, 0))]), dim=0)
        self.data = self.data * self.mask
        self.data = self.data.view(len(self), self.seq_len, -1).contiguous()
