...
def _generate_prediction_raw_mask(self):
    raw_mask = torch.stack(tuple(
        [torch.cat((self.generate_raw_mask_sample(self), self.generate_raw_mask_sample(self)), dim=0) for _ in
...
class LosAngelesHighwaySpeed(Dataset):
    def __init__(self, raw_data_dir, raw_adjacency_dir, missing_type, missing_rate, num_timestamp, train_proportion,
        ...
        self.data = [torch.cat((self.past_data[index].x.view(self.seq_len, self.graph_size),
                                self.future_data[index].x.view(self.seq_len, self.graph_size)), dim=0)
                     for index in range(len(self))]

        self.mask = [torch.cat((self.past_mask[index].x.view(self.seq_len, self.graph_size),
                                self.future_mask[index].x.view(self.seq_len, self.graph_size)), dim=0)
                     for index in range(len(self))]
