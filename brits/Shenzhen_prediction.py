...
train_set = LosAngelesHighwaySpeedPredictionTrainSet(raw_data_dir="./datasets/Shenzhen_data.npy",
...
valid_set = LosAngelesHighwaySpeedPredictionTestSet(raw_data_dir="./datasets/Shenzhen_data.npy",
                                                    raw_adjacency_dir="./datasets/Shenzhen_adjacency.npy",
                                                    missing_type=args.missing_type, missing_rate=args.missing_rate,
                                                    num_timestamp=2970, train_proportion=0.95,
                                                    seq_len=8, graph_size=165, feature_channels=1)
...
valid_mask_set = LosAngelesHighwaySpeedPredictionTestSet(raw_data_dir="./datasets/Shenzhen_data.npy",
                                                         raw_adjacency_dir="./datasets/Shenzhen_adjacency.npy",
                                                         missing_type=args.missing_type, missing_rate=args.missing_rate,
                                                         num_timestamp=2970, train_proportion=0.95,
                                                         seq_len=8, graph_size=165, feature_channels=1)
...
version = 1
dataset_name = "Predict_Shenzhen_data_v{}".format(version)
...
for epoch in range(args.epochs):
    ...
    with torch.no_grad():
        mask_zero = torch.zeros(args.batch_size, args.seq_len, 165)
        if args.cuda_is_available:
        ...
        for batch, batch_for_mask in valid_bar:
            ...
            # print(mask_zero.shape, (torch.numel(mask_zero) - mask_zero.sum().item()))
            # print(mask.shape, (torch.numel(mask) - mask.sum().item()))
