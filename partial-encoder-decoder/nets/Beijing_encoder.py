...
import nets.flow as flow


class ConvEncoder(nn.Module):
    def __init__(self, latent_size, flow_depth=2, logprob=False, input_channel=1):
        super().__init__()
        ...
        self.main = nn.Sequential(
            nn.Conv2d(self.input_channel, DIM, 5, stride=2, padding=2),
            nn.ReLU(True),
    ...
    def encode(self, input, _):
        x = self.main(input.view(-1, self.input_channel, 32, 32))
        x = x.view(input.shape[0], -1)
        fc_out = self.fc(x).chunk(self.enc_chunk, dim=1)
        mu, logvar = fc_out[:2]
        std = F.softplus(logvar)
