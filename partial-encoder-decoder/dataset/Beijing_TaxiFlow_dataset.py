...
def load_train_data(self):
	...
	raw_data = raw_data[:self.num_data_timestamp, :, :, :1]
	L, H, W, F = raw_data.shape
	raw_data = raw_data.reshape(L, H * W * F)
	scaler = preprocessing.MinMaxScaler()
	raw_data = scaler.fit_transform(raw_data).reshape(L, H, W, F)
	self.scaler = scaler
...
def get_prediction_item(self, index):
	data_sample = self.data[index:index + self.seq_len * 2, :, :, :].view(
				self.seq_len * 2, self.grid_size_x, self.grid_size_y)
	mask_sample = self.mask[index].view(self.seq_len * 2, self.grid_size_x, self.grid_size_y)
	return data_sample, mask_sample
